...
// Step 21: Caesar's Traditional Secret Party - Create a function to encrypt and decrypt messages
function caesarCipherMessage(message, shift, encrypt = true) {
  const shiftValue = encrypt ? shift : -shift;
  return message.split('').map(char => {
    const charCode = char.charCodeAt(0);
    if (charCode >= 65 && charCode <= 90) {
      return String.fromCharCode(((charCode - 65 + shiftValue + 26) % 26) + 65);
    } else if (charCode >= 97 && charCode <= 122) {
      return String.fromCharCode(((charCode - 97 + shiftValue + 26) % 26) + 97);
    }
    return char;
  }).join('');
}

const secretMessage = "Meet at the Forum";
const encryptedSecretMessage = caesarCipherMessage(secretMessage, secretShiftValue);
console.log(encryptedSecretMessage); // Encrypted secret message

const decryptedSecretMessage = caesarCipherMessage(encryptedSecretMessage, secretShiftValue, false);
console.log(decryptedSecretMessage); // Decrypted secret message

// Answer to question in a comment
// The function caesarCipherMessage can encrypt or decrypt a message based on the provided shift value and the encrypt flag. By default, it encrypts the message; if the encrypt flag is set to false, it decrypts the message.
